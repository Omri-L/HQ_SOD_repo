...
class Kernel_DKN(nn.Module):
    def __init__(self, input_channel, kernel_size, device='cpu'):
...
class DKN(nn.Module):
    def __init__(self, kernel_size, filter_size, residual=True, device='cpu'):
        super(DKN, self).__init__()
        self.device = device
        self.ImageKernel = Kernel_DKN(input_channel=3, kernel_size=kernel_size, device=self.device)
        self.DepthKernel = Kernel_DKN(input_channel=1, kernel_size=kernel_size, device=self.device)
...
class Kernel_FDKN(nn.Module):
    def __init__(self, input_channel, kernel_size, factor=4, device='cpu'):
...
class FDKN(nn.Module):
    def __init__(self, kernel_size, filter_size, residual=True, device='cpu'):
        ...
        self.device = device
        self.ImageKernel = Kernel_FDKN(input_channel=16, kernel_size=kernel_size, factor=self.factor, device=device)
        self.DepthKernel = Kernel_FDKN(input_channel=16, kernel_size=kernel_size, factor=self.factor, device=device)
        self.residual = residual
        self.kernel_size = kernel_size
        self.filter_size = filter_size
        self.sigmoid = torch.nn.Sigmoid()
